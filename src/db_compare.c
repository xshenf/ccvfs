...
static int compare_databases(const char *db1_path, const char *db2_path, 
                            const CompareOptions *options, CompareResult *result) {
...
static void print_results(const CompareResult *result, const CompareOptions *options) {
...
int main(int argc, char *argv[]) {
    ...
    int c;
    while ((c = getopt_long(argc, argv, "siwt:vh", long_options, NULL)) != -1) {
        switch (c) {
            case 's':
                options.compare_schema_only = 1;
                break;
            case 'i':
    ...
    if (optind + 2 != argc) {
        fprintf(stderr, "错误: 需要指定两个数据库文件\n");
        print_usage(argv[0]);
        return 1;
    }
    
    const char *db1_path = argv[optind];
    const char *db2_path = argv[optind + 1];
    
    // Initialize SQLite and CCVFS
    sqlite3_initialize();
    
    // Register CCVFS for handling compressed databases
    rc = sqlite3_ccvfs_create("ccvfs", NULL, "zlib", NULL, 0, 0);
    if (rc != SQLITE_OK && options.verbose) {
